...
### Command: "command" / "command menu" / "command center menu"
When the user asks for the command center menu, reply with:

```
🎯 *Command Center*

🔟 *Dashboard* — Open on your phone
1️⃣1️⃣ *Add task* — Quick add a task
1️⃣2️⃣ *My tasks* — Show active tasks
1️⃣3️⃣ *Shopping list* — Show shopping list
1️⃣4️⃣ *Briefing* — Today's overview
1️⃣5️⃣ *Reminders* — Active reminders
1️⃣6️⃣ *Command help* — Show this menu

💡 _Or just tell me what you need naturally_
```

### Number shortcuts for Command Center (10-16)
If the user sends a number 10-16, treat it as selecting that option:
- **10** → Send dashboard link: `📊 *Command Center*\n\nhttp://{LOCAL_IP}:3005\n\n_Tap to open_`
  - Get local IP via: `python3 -c "import socket; s=socket.socket(socket.AF_INET,socket.SOCK_DGRAM); s.connect(('8.8.8.8',80)); print(s.getsockname()[0])"`
  - Make sure dev server is running on port 3005
- **11** → Reply "What's the task?" then create via `/api/tasks` with their next reply
- **12** → Fetch tasks from `/api/tasks` and send formatted list
- **13** → Fetch from `/api/shopping` and send formatted list
- **14** → Run briefing (calendar + tasks + reminders summary)
- **15** → Fetch from `/api/reminders?status=pending` and send formatted list
- **16** → Show command center menu again

### Command: "dashboard" / "dash" / "open dashboard"
Shortcut — same as option 10 above. Send the dashboard link immediately.

### Priority routing

1. Check if message matches tradies bookkeeping triggers FIRST (receipt photo with "test receipt", "who owes me", "chase", "monthly summary", "menu", numbers 1-6)
2. Check if message matches command center triggers ("command", "command menu", "dash", "dashboard", numbers 10-16)
3. If no match → route to Command Center (`/api/command`)
4. If Command Center returns `needsClassification` → use your own judgment to classify and act
